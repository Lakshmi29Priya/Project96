...
export default class MyDonationScreen extends Component {
   ...
   sendBook=(bookDetails)=>{
     if(bookDetails.request_status === "Gift Sent"){
     ...
     .then((snapshot)=>{
       snapshot.forEach((doc) => {
         ...
         if(requestStatus === "Gift Sent"){
           message = this.state.donorName + " sent you gift"
         }else{
            message =  this.state.donorName  + " has shown interest in giving the gift"
         }
       ...
       rightElement={
           <TouchableOpacity
            style={[
              styles.button,
              {
                backgroundColor : item.request_status === "Gift Sent" ? "green" : "#ff5722"
              }
            ]}
            onPress = {()=>{
              this.sendBook(item)
   ...
   render(){
     return(
       <View style={{flex:1}}>
         <MyHeader navigation={this.props.navigation} title="My Gifts"/>
         <View style={{flex:1}}>
           {
